...
Install the package:
```bash
pip install --extra-index-url=https://test.pypi.org/simple/ ak-oo-patterns
```
...
```bash
git clone https://github.com/ABKorotky/oo-patterns.git
cd ak-oo-patterns
```
...
- `ann`. Annotation. Checks type annotations in the project using `mypy` tool.
- `utc`. Unit Tests with Coverage. Runs the project's unit tests and calculates the level of coverage.
...
- `main` branch is default stable branch. It is used for releasing new stable distributions.
...
6. Write what you have done in `CHANGELOG.md` file into the corresponding section. Remember to actualize the `VERSION` of these changes in `ak_oo_patterns/__init__.py` file.
7. Examine the diff before merging, clean it from accidentally committed garbage.
...
12. Prepare a distribution from the tag. Run `tox -e build`. Make final checks with a generated archive. Ensure it is installed properly.
13. Publish the archive in Test PyPI. Run `tox -e upload -- dist/ak-oo-patterns-<version>.tar.gz`. Ensure that the distribution is installed properly from Test PyPI. Prepare a temporary environment and run `pip install --extra-index-url=https://test.pypi.org/simple/ ak-oo-patterns`.
14. Publish the archive in main PyPI. Run `export PYPI_REPOSITORY_ALIAS=pypi` for switching uploading to main PyPI and repeat uploading via `tox -e upload -- ...`. Ensure that the distribution is installed properly from PyPI. Prepare a temporary environment and run `pip install ak-oo-patterns`.
